...
function processParameters(heatpumpParameters, heatpumpVisibility) {
    ...
    return {
        ...
        // Controllers that report fewer than 876 parameters yield undefined here.
        // Upstream calls .toString() on it unguarded, and because processParameters
        // runs from process.nextTick the resulting TypeError is an uncaught
        // exception rather than a callback error, so it takes the whole app down.
        typeSerial: (function () {
            const buildDate = heatpumpParameters[874];
            const serialNumber = heatpumpParameters[875];
            if (buildDate === undefined || serialNumber === undefined) {
                return null;
            }
            return (
                buildDate.toString().substr(0, 4) +
                '/' +
    ...
    this.client.on(
        ...
        function (data) {
            if (this.receivy.activeCommand === 0) {
                this.dataBuffer = this.dataBuffer === undefined
                    ? data
                    : Buffer.concat([this.dataBuffer, data]);
            }

            if (this.dataBuffer.length >= 8 || this.receivy.activeCommand !== 0) {
                if (this.receivy.activeCommand === 0) {
                    ...
                    // Header not fully arrived yet: wait for the next segment.
