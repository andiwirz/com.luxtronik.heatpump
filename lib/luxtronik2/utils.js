...
function secondsToTimeString(value) {
    ...
    // The timer tables read parameters 223..606. Controllers that report fewer
    // yield undefined, so the Date is invalid and toISOString() throws a
    // RangeError. processParameters runs from process.nextTick, which makes that
    // an uncaught exception rather than a callback error, taking the app down.
    if (Number.isNaN(timeMilliseconds.getTime())) {
        return '00:00';
    }
